...
def load_key(filename):
    """
    What it does: obtains a key from a file.
    
    Parameters:
        filename: the name of the file that contains the key.
        
    Returns:
        The RSA key loaded from the file.
...
def generate_sym_key():
    """
    Generates and returns an AES 256 symmetric key.
    """
...
def send(sock: socket.socket, message: bytes):
    """Sends raw data to the client with a length header."""
...
def handle_send_email(conn, username, sym_key):
    ...
    if parsed_email is None:
        print("Invalid email format. Email rejected.")
        return

    if parsed_email["from"] != username:
        print("Invalid sender. Email rejected.")
        return

    destinations = parsed_email["to"]
    title = parsed_email["title"]
    length = parsed_email["content_length"]
    content = parsed_email["content"]

    print(f"An email from {username} is sent to {destinations} and has a content length of {length}.")
...
def handle_client(conn, users):
    ...
    public_key = load_key(f"{username}_public.pem")
    encrypted_key = rsa_encrypt(public_key, sym_key)
    send(conn, encrypted_key)
